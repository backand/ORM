...
To retrieve all fields of employees with ages beteen 25 and 40, 

    { object: "employees", q: { age : { $between : [25, 40] } }  } 
...
2. Comparison of a field using a comparison operator, e.g. `{ A: { $gt: 8 }}`. Is `A` greater than 8? The set of comparison operators is quite extensive and includes: `$lte, $lt, $gte, $gt, $eq, $neq, $not, $within, $between`
